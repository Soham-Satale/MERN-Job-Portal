...
const JobCard = ({job,onClick,onToggleSave,onApply,saved,hideApply}) => {
  ...
  return (
    <div 
    className='bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-xl hover:shadow-gray-200 transition-all duration-300 group relative overflow-hiddencursor-pointer'
    onClick={onClick}
    >
        <div className='flex items-start justify-between mb-4'>
            <div className='flex items-start gap-4'>
                {job?.company?.companyLogo ? (
                    ...
                    alt="company logo" 
                    className='w-14 h-14 rounded-2xl object-cover border-4 border-white/20 shadow-lg'  
                    />
                ) : (
